...
- facades: 各立面详细描述数组。**每个立面描述该面可见的结构**，包括：
  * face: 立面方向，"front"(正面), "back"(背面), "left"(左侧), "right"(右侧)
  * material: 该面墙材料（与全局 materials 格式相同，空表示同全局）
  * columns: 立柱水平位置列表（0~1 比例）如 [0.1, 0.3, 0.5, 0.7, 0.9]
  * windows: 窗户数组，每项含 x(水平位置0~1), width(宽度0~1), height(高度，方块数), y_offset(离地高度)
  * openings: 开口数组（门洞等），每项含 x(水平位置0~1), width(宽度0~1), height(高度，方块数), style("rectangle"方/"arch"拱)
  * railings: 是否有栏杆/护栏
  * cornice: 是否有檐口线脚
  注意：如果只能看到正面，至少填写 front 面，其他面可不填
...
def analyze(
    ...
    # --- 解析 facades ---
    facades = []
    for f in raw.get("facades", []):
        if isinstance(f, dict):
            windows = []
            for w in f.get("windows", []):
                if isinstance(w, dict):
                    windows.append(FaceWindow(**w))
            openings = []
            for o in f.get("openings", []):
                if isinstance(o, dict):
                    openings.append(FaceOpening(**o))
            facades.append(Facade(
                face=f.get("face", "front"),
                material=f.get("material", ""),
                columns=f.get("columns", []),
                windows=windows,
                openings=openings,
                railings=f.get("railings", False),
                cornice=f.get("cornice", False),
            ))
